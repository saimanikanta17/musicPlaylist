...
class MusicPlaylist extends Component {
  ...
  changePlayList = id => {
    ...
    const filteredTrackList = tracksList.filter(each => each.id !== id)
  ...
  render() {
    ...
    const searchResults = tracksList.filter(eachSearch =>
      eachSearch.name.toLowerCase().includes(searchValue.toLowerCase()),
    )
    const showPlaylist = searchResults.length === 0
    return (
